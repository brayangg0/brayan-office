  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button onClick={() => { setForm({ contactId: '', courseId: '', cpf: '', rg: '', expiresAt: '' }); setModal({ open: true }); }} className="btn-primary flex items-center gap-2 self-start sm:self-auto">
            ...
            <div key={s.id} className="card !p-4 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="font-semibold text-sm">{s.contact.name}</p>
                  <p className="text-xs text-gray-500">{s.contact.phone}</p>
                  {s.contact.email && <p className="text-xs text-gray-400">{s.contact.email}</p>}
                </div>
                <button onClick={() => { setForm({ contactId: s.contactId, courseId: s.courseId || '', cpf: s.cpf || '', rg: s.rg || '', expiresAt: s.expiresAt?.slice(0, 10) || '' }); setModal({ open: true, student: s }); }}
                  className="text-xs text-blue-500 hover:text-blue-700 shrink-0 px-2 py-1 border border-blue-200 rounded-lg">Editar</button>
              </div>
              <div className="flex flex-wrap gap-1.5 text-xs">
                <span className={`badge ${(statusColor as any)[s.status] || 'bg-gray-100 text-gray-600'}`}>{s.status}</span>
                {s.course?.name && <span className="badge bg-purple-100 text-purple-700">{s.course.name}</span>}
                {s.expiresAt && (
                  <span className={`flex items-center gap-1 badge ${expired ? 'bg-red-100 text-red-600' : expiringSoon ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 text-gray-500'}`}>
                    {(expired || expiringSoon) && <AlertTriangle size={10} />}
                    {format(new Date(s.expiresAt), 'dd/MM/yyyy')}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-3 text-xs">
                {s.cpf && <span className="text-gray-500">CPF: {s.cpf}</span>}
                {s.rgPhotoPath
                  ? <a href={s.rgPhotoPath} target="_blank" className="text-whatsapp flex items-center gap-1"><Eye size={12} /> Ver RG</a>
                  : <button onClick={() => setRgModal({ open: true, studentId: s.id })} className="text-blue-500 flex items-center gap-1"><Upload size={12} /> Enviar RG</button>
                }
              </div>
            </div>
          );
        })}
        {data && <p className="text-xs text-gray-400 text-center">{data.total} alunos</p>}
      </div>
      ...
      {modal.open && (
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-xl w-full sm:max-w-md">
            ...
            <form onSubmit={(e) => { e.preventDefault(); saveMut.mutate(form); }} className="p-4 space-y-3 max-h-[80vh] overflow-y-auto">
              {!modal.student && (
                <div>
